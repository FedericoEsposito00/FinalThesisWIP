...
void Estimator::step()
{
  ...
  for (i_0 = 0; i_0 < 6; i_0++) {
    ...
    rtb_Gain[i_0] = (ct_0 - rtDW.DiscreteTimeIntegrator_DSTATE[i_0]) * 1000.0;

    // Outport: '<Root>/estimate' incorporates:
    //   DiscreteIntegrator: '<S1>/Estimate integrator'

    rtY.estimate[i_0] = rtDW.Estimateintegrator_DSTATE[i_0];

    // Gain: '<S1>/Gain1' incorporates:
    //   DiscreteIntegrator: '<S1>/Estimate integrator'

    rtb_Gain1[i_0] = 300.0 * rtDW.Estimateintegrator_DSTATE[i_0];
  }
  ...
  for (int32_T i{0}; i < 3; i++) {
    ...
    tmp_1[i] = (20.269 * b_0[i] - ((a[i + 3] * rtU.u[1] + a[i] * rtU.u[0]) + a[i
      + 6] * rtU.u[2])) + rtDW.Estimateintegrator_DSTATE[i];
  ...
  for (int32_T i{0}; i <= 0; i += 2) {
    ...
    tmp_7 = _mm_loadu_pd(&rtDW.Estimateintegrator_DSTATE[i + 3]);
  ...
  for (int32_T i{2}; i < 3; i++) {
    ...
    tmp_0[i + 3] = ((((Q_tmp_1[i + 3] * rtU.eta_dot[1] + Q_tmp_1[i] *
                    ...
                    rtDW.Estimateintegrator_DSTATE[i + 3]) * 0.01 +
  ...
  for (int32_T i{0}; i <= 4; i += 2) {
    ...
    // DiscreteIntegrator: '<S1>/Estimate integrator' incorporates:
    //   DiscreteIntegrator: '<S1>/Discrete-Time Integrator'

    tmp_9 = _mm_loadu_pd(&rtDW.Estimateintegrator_DSTATE[i]);

    // DiscreteIntegrator: '<S1>/Discrete-Time Integrator1' incorporates:
    //   DiscreteIntegrator: '<S1>/Discrete-Time Integrator'

    tmp_a = _mm_loadu_pd(&rtDW.DiscreteTimeIntegrator1_DSTATE[i]);

    // Update for DiscreteIntegrator: '<S1>/Estimate integrator'
    tmp_2 = _mm_set1_pd(0.01);

    // DiscreteIntegrator: '<S1>/Estimate integrator' incorporates:
    //   DiscreteIntegrator: '<S1>/Discrete-Time Integrator'

    tmp_3 = _mm_loadu_pd(&rtDW.Estimateintegrator_DSTATE[i]);

    // Update for DiscreteIntegrator: '<S1>/Estimate integrator' incorporates:
    //   DiscreteIntegrator: '<S1>/Discrete-Time Integrator'
    //   Gain: '<S1>/Gain2'

    _mm_storeu_pd(&rtDW.Estimateintegrator_DSTATE[i], _mm_add_pd(_mm_mul_pd
      (_mm_sub_pd(tmp_a, _mm_mul_pd(_mm_set1_pd(30.0), tmp_9)), tmp_2), tmp_3));

    // Sum: '<S1>/Subtract1' incorporates:
    //   DiscreteIntegrator: '<S1>/Discrete-Time Integrator'

    tmp_9 = _mm_loadu_pd(&rtb_Gain[i]);
    tmp_a = _mm_loadu_pd(&rtb_Gain1[i]);

    // DiscreteIntegrator: '<S1>/Discrete-Time Integrator1' incorporates:
    //   DiscreteIntegrator: '<S1>/Discrete-Time Integrator'

    tmp_3 = _mm_loadu_pd(&rtDW.DiscreteTimeIntegrator1_DSTATE[i]);
    ...
    _mm_storeu_pd(&rtDW.DiscreteTimeIntegrator1_DSTATE[i], _mm_add_pd(_mm_mul_pd
      (_mm_sub_pd(tmp_9, tmp_a), tmp_2), tmp_3));
