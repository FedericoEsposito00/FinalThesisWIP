...
LEFT_INVKIN::LEFT_INVKIN() {
	...
	_js_sub = _nh.subscribe("/licasa1/joint_states", 0, &LEFT_INVKIN::joint_states_cb, this);
	
	//Output: the cartesian position of the end-effector
	_cartpose_pub = _nh.advertise<geometry_msgs::Pose>("/licasa1/left_eef_pose", 0);
	//Output: the command to the robot joints
	_cmd_pub[0] = _nh.advertise< std_msgs::Float64 > ("/licasa1/licasa1_leftarm_1_effort_pos_controller/command", 1);
	_cmd_pub[1] = _nh.advertise< std_msgs::Float64 > ("/licasa1/licasa1_leftarm_2_effort_pos_controller/command", 1);
	_cmd_pub[2] = _nh.advertise< std_msgs::Float64 > ("/licasa1/licasa1_leftarm_3_effort_pos_controller/command", 1);
	_cmd_pub[3] = _nh.advertise< std_msgs::Float64 > ("/licasa1/licasa1_leftarm_4_effort_pos_controller/command", 1);
...
bool LEFT_INVKIN::init_robot_model() {
	...
	std::string tip_link  = "left_eef_link";
	if ( !_k_tree.getChain(base_link, tip_link, _left_k_chain) ) return false;

	cout<<"Joints in left chain: "<<_left_k_chain.getNrOfJoints()<<endl;
	...
	_fksolver = new KDL::ChainFkSolverPos_recursive( _left_k_chain );
	_ik_solver_vel = new KDL::ChainIkSolverVel_pinv( _left_k_chain );
	...
	//and the allowed errors on the task space and on the joint positioning (left at default)
	...
	_ik_solver_pos = new KDL::ChainIkSolverPos_LMA( _left_k_chain, _L);
	...
	_q_in = new KDL::JntArray( _left_k_chain.getNrOfJoints() );
...
void LEFT_INVKIN::joint_states_cb( sensor_msgs::JointState js ) {
...
void LEFT_INVKIN::goto_initial_position( float dp[NJ] ) {
...
void LEFT_INVKIN::get_dirkin() {

	ros::Rate r(100);

	KDL::JntArray q_curr(_left_k_chain.getNrOfJoints());
...
void LEFT_INVKIN::ctrl_loop() {
	...
	KDL::JntArray q_out(_left_k_chain.getNrOfJoints());

	while (!_listener.waitForTransform("shoulder_link_y", "left_ref_frame", ros::Time(0), ros::Duration(1))){
		sleep(1);
	}
	...
	while(ros::ok()) {
		...
		_listener.lookupTransform("shoulder_link_y","left_ref_frame",ros::Time(0), _tf_ref);
...
void LEFT_INVKIN::run() {

	//In the run method, we start the threads to:
	//	- Calculate the forward kinematic
	//	- Calculate the inverse kinematic 
	std::thread get_dirkin_t( &LEFT_INVKIN::get_dirkin, this);
	std::thread ctrl_loop_t ( &LEFT_INVKIN::ctrl_loop, this);
	ros::spin();	
...
int main(int argc, char** argv) {
	...
	ros::init(argc, argv, "left_arm_kdl");
	LEFT_INVKIN ik;
	ik.run();
