...
class RIGHT_INVKIN {
	public:
		...
		void goto_initial_position( float dp[NJ] );
	...
	private:
		...
		//Forward kinematics solver_ik_solver_pos;
		...
		KDL::ChainIkSolverPos_LMA *_ik_solver_pos;

		ros::Subscriber _js_sub;
		ros::Publisher _cartpose_pub;
		ros::Publisher _cmd_pub[NJ];
		...
		//Tf objects to read the reference
		tf::TransformListener _listener;
		tf::StampedTransform _tf_ref;

};

RIGHT_INVKIN::RIGHT_INVKIN() {
	...
	cout << "Joints and segments: " << _k_tree.getNrOfJoints() << " - " << _k_tree.getNrOfSegments() << endl;
 
	//Input: the current configuration of the robot (its joints value)
	_js_sub = _nh.subscribe("/licasa1/joint_states", 0, &RIGHT_INVKIN::joint_states_cb, this);
	
	//Output: the cartesian position of the end-effector
	_cartpose_pub = _nh.advertise<geometry_msgs::Pose>("/licasa1/right_eef_pose", 0);
	//Output: the command to the robot joints
	_cmd_pub[0] = _nh.advertise< std_msgs::Float64 > ("/licasa1/licasa1_rightarm_1_effort_pos_controller/command", 1);
	_cmd_pub[1] = _nh.advertise< std_msgs::Float64 > ("/licasa1/licasa1_rightarm_2_effort_pos_controller/command", 1);
	_cmd_pub[2] = _nh.advertise< std_msgs::Float64 > ("/licasa1/licasa1_rightarm_3_effort_pos_controller/command", 1);
	_cmd_pub[3] = _nh.advertise< std_msgs::Float64 > ("/licasa1/licasa1_rightarm_4_effort_pos_controller/command", 1);
...
bool RIGHT_INVKIN::init_robot_model() {
	...
	std::string tip_link  = "right_eef_link";
	if ( !_k_tree.getChain(base_link, tip_link, _right_k_chain) ) {
		cout<<"Error getting chain\n";
		return false;
	}

	cout<<"Joints in right chain: "<<_right_k_chain.getNrOfJoints()<<endl;
	...
	_fksolver = new KDL::ChainFkSolverPos_recursive( _right_k_chain );
	_ik_solver_vel = new KDL::ChainIkSolverVel_pinv( _right_k_chain );
	//The invers kinematic solver object needs must be initialized considering also
	//the weights of the DOFs to solve the ik problem on a given robot configuration
	//and the allowed errors on the task space and on the joint positioning (right at default)
	...
	_ik_solver_pos = new KDL::ChainIkSolverPos_LMA( _right_k_chain, _L);
	...
	_q_in = new KDL::JntArray( _right_k_chain.getNrOfJoints() );
...
void RIGHT_INVKIN::joint_states_cb( sensor_msgs::JointState js ) {

	//We assume to know the number of joints
	for(int i=0; i<NJ; i++ ) {
		_q_in->data[i] = js.position[i+NJ];
	}
...
void RIGHT_INVKIN::goto_initial_position( float dp[NJ] ) {
...
void RIGHT_INVKIN::get_dirkin() {

	ros::Rate r(100);

	KDL::JntArray q_curr(_right_k_chain.getNrOfJoints());
...
void RIGHT_INVKIN::ctrl_loop() {
	...
	while (!_listener.waitForTransform("shoulder_link_y", "right_ref_frame", ros::Time(0), ros::Duration(1))){
		sleep(1);
	}
	...
	while(ros::ok()) {
		...
		_listener.lookupTransform("shoulder_link_y","right_ref_frame",ros::Time(0), _tf_ref);

		F_dest.p.data[0] = _tf_ref.getOrigin().x();
		F_dest.p.data[1] = _tf_ref.getOrigin().y();
		...
		int err_code = _ik_solver_pos->CartToJnt(*_q_in, F_dest, q_out);

		if( err_code != KDL::SolverI::E_NOERROR ) 
			cout << "failing in ik with code " << err_code << endl;
...
void RIGHT_INVKIN::run() {

	//In the run method, we start the threads to:
	//	- Calculate the forward kinematic
	//	- Calculate the inverse kinematic 
	std::thread get_dirkin_t( &RIGHT_INVKIN::get_dirkin, this);
	std::thread ctrl_loop_t ( &RIGHT_INVKIN::ctrl_loop, this);
	ros::spin();	
...
int main(int argc, char** argv) {
	...
	ros::init(argc, argv, "right_arm_kdl");
	RIGHT_INVKIN ik;
