...
void RIGHT_INVKIN::ctrl_loop() {
	...
	string pkg_loc = ros::package::getPath("cable-suspended-robots-ros-pkg");
	ref.open(pkg_loc + "/right_ref.txt", ios::in); 

	if (!ref.is_open()) {
		cout<<"Error opening the right file!";
		exit(1);
	}
	...
	*/
	...
	while(ros::ok()) {
		...
		if (j<N-1) {
			...
			j = 0;
			ROS_INFO("RESTARTING RIGHT TRAJECTORY\n");
		}
...
int main(int argc, char** argv) {
	...
	ros::init(argc, argv, "right_arm_kdl_file");
	RIGHT_INVKIN ik;
